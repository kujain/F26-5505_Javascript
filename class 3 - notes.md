...
function calc_avg ( arr ) {
	...
	// using loops - option 1
    for ( const val of arr ) {
        total = total + val;
    }

	// using an array iterator - option 2
    total = arr.reduce( ( prev, curr ) =>  prev + curr );

    let avg = total / arr.length;

	// the above 2 lines could be combined as well:
...
function findMatch( string, pattern ) {
    ...
    string = string.toLowerCase(); // convert all to lowercase for better matching
